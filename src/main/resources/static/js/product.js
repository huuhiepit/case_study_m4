...
async function getCompaniesSelectOption() {
...
window.onload = async () => {
    ...
    companies = await getCompaniesSelectOption();
    await renderTable()
...
function getDataInput() {
    return [
        ...
        },
        {
            label: 'Company',
            name: 'companyId',
            value: productSelected.companyId,
            type: 'select',
            required: true,
            options: companies,
...
function renderItemStr(item, index) {
    return `<tr>
                    ...
                    <td>
                         <div class="client">
                            <div class="client-img bg-img" style="background-image: url(${item.urlImage})"></div>                 
